...
class DetailSurat extends StatelessWidget {
  ...
  String? asmaSurat;
  String? artiSurat;
  DetailSurat({ Key? key, required this.nomor, required this.namaSurat, required this.asmaSurat, required this.artiSurat }) : super(key: key);
                          ...
                          onLongPress: (){
                            showDialog(context: context, builder: (context){
                              ...
                              actions: <Widget>[
                                MaterialButton(
                                  onPressed: () async {
                                    var input = SuratModel(nomor: nomor! , nama: namaSurat!, asma: asmaSurat!, arti: artiSurat!, ayat:snapshot.data![i].ayat, teks: snapshot.data![i].teks);
                                    // insert data to bookmark
                                    helper = SQLiteHelper();
                                    helper!.inisiasiDB().whenComplete(() async {
                                      await helper!.insertSurat(input);
                                    });
                                    Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>Bookmark()));
                                  },
                                  color: Colors.white,
                                  elevation: 0,
                                  child: Text("Yes",style: TextStyle(color: HexColor("#B068D0")),),
                                ),
                                MaterialButton(
                                  onPressed: (){
                                  ...
                                  color: Colors.white,
                                  elevation: 0,
                                  child: Text("No",style: TextStyle(color: HexColor("#B068D0")),),
                                )
                              ],
                            );
                            });
